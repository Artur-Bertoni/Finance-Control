...
export function init() {
    ...
    if (!categoryId) { navigate('/pages/lists/CategoryList.html'); return }

    const response = doRequest(`/api/categories/${categoryId}`, 'GET')
    if (!response?.id) { navigate('/pages/lists/CategoryList.html'); return }
    ...
    document.getElementById('edit-btn').addEventListener('click', () =>
        navigate(`/pages/crud/Category.html?id=${categoryId}`)
    )
    ...
    document.querySelectorAll('.view-tab').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (tab === 'history' && !historyLoaded) {
                historyLoaded = true
                ChangeHistoryManager.loadAndRender('category', categoryId, response.createdAt, 'history-container')
            }
        })
    })
}
