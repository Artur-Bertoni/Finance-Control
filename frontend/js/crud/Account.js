...
function handleSave(accountId) {
    ...
    $.ajax({
        ...
        success: (data) => {
            ...
            if (accountId) {
                navigate(`/pages/views/AccountView.html?id=${id}`)
            } else {
                navigateWithToast('/pages/lists/AccountList.html', I18n.t('accountCreatedSuccess'), 'success', id ? `/pages/views/AccountView.html?id=${id}` : null)
            }
        },
